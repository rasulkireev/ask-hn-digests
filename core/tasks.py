...
class HNDiscussionSummaryResponse(BaseModel):
    short_summary: str
    long_summary: str
    title: str
    slug: str
...
summarize_agent = Agent(
    ...
    output_type=HNDiscussionSummaryResponse,
    system_prompt="""You are an expert at analyzing Hacker News discussions and creating high-quality summaries.
...
- Do not mention HN or Hacker News in any of the outputs.
- Do not talk about the discussion in summaries. Write as if you are writing original content with the discussion as a reference.""",  # noqa: E501
)
...
Provide your analysis with the following requirements:
...
"""  # noqa: E501
    ...
    result = run_agent_synchronously(
        summarize_agent,
        prompt,
        function_name="summarize_hn_discussion",
...
def generate_blog_post(summary: HNDiscussionSummary):
    """
    Generates a blog post for the given HNDiscussionSummary.
    """
    summary.generate_blog_post()
    return "Success"
...
def generate_summary_tags(summary: HNDiscussionSummary):
    ...
    try:
        ...
        if generated_tags:
            ...
            async_task("core.tasks.generate_blog_post", summary, group="Generate Blog Post")
            return "Success"
        else:
            logger.error(
